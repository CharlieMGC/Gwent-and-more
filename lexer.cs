                    ...
                    else
                    {
                        ...
                        if (identifier == "Console" && i < buffer.Length - 1 && buffer[i] == '.')
                        {
                            ...
                            if (i + 8 <= buffer.Length && new string(buffer, i, 8) == "WriteLine")
                            {
                                ...
                                tokens.Add(new Token(TokenType.CONSOLE_WRITELINE, currentToken.ToString(), null, line, column - currentToken.Length));
                                i += 8;
                                column += 8;
                            }
                            else
                            {
                                // If it's not "WriteLine", treat it as a normal identifier
                                tokens.Add(new Token(TokenType.ID, identifier, null, line, column - identifier.Length));
                                i--;
                                column--;
                            }
                        }
                        else if (keywords.TryGetValue(identifier, out TokenType keywordType))
                        {
